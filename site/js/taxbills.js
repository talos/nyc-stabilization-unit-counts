...
(function () {
  ...
  function showGraphs(data) {
    ...
    var rentStabilization = data.table.where(function (t) {
      return t.key === 'Housing-Rent Stabilization';
    }).groupBy(function (t) {
      return t.dueDate;
    }).orderBy(function (t) {
      return new Date(t[0].dueDate);
    }).select(function (t) {
      var sumapts = 0,
          regnos = {};
      for (var i = 0; i < t.length; i += 1) {
        if (!(t[i].meta in regnos)) {
          sumapts += Number(t[i].apts);
          regnos[t[i].meta] = true;
        }
      }

      return {x: new Date(t[0].dueDate).getFullYear(), y: sumapts};
    });
    ...
    .axisLabel('Registration Year');
    //.tickFormat(d3.format(',r'));

    chart.yAxis     //Chart y-axis settings
    .axisLabel('Rent Stabilized Units');
    //.tickFormat(d3.format('.02f'));

    /* Done setting the chart up? Time to render it!*/

    d3.select('#graphs svg')    //Select the <svg> element you want to render the chart in.   
    .datum([{
      values: rentStabilization,
      key: 'Rent Stabilized Units',
      color: '#000'
